...
`odigos-system`) with `networkMetrics.enabled: true`. See [Network Metrics](/oss/pipeline/rules/networkmetrics) for configuration options,
...
infrastructure with `metricsSources.networkMetrics.disabled: false`, then enable collection with an
InstrumentationRule `networkMetrics.enabled: true` toggle (cluster-wide or scoped via `scopes`).
